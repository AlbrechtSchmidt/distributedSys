...
main()
{
	...
	if (!ioctl(0, I_LOOK, mname) &&
		...
		if (!svc_reg(transp, MY_DATE_PROG, DATE_VERS, my_date_prog_1, 0)) {
			_msgout("unable to register (MY_DATE_PROG, DATE_VERS).");
			exit(1);
		}
		if (pmclose) {
			(void) signal(SIGALRM, (void(*)()) closedown);
	...
	if (!svc_create(my_date_prog_1, MY_DATE_PROG, DATE_VERS, "netpath")) {
		_msgout("unable to create (MY_DATE_PROG, DATE_VERS) for netpath.");
		exit(1);
	}
